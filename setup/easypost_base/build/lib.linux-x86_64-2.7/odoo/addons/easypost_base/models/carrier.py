...
def set_field_ep_carriers_selection(env, field):
    ...
    if env.cr.fetchall():
        ...
        for (code, _name) in field.selection:
            if (
                code.startswith(EP_PREFIX)
                and code != EP_PREFIX
                and code not in ep_carriers.mapped("code")
...
class EasypostCarrier(models.Model):
    ...
    delivery_date_guaranteed = fields.Boolean(
        string="Guaranteed Delivery Date",
        help="Check this box if you want the system to select a service with a " "guaranteed delivery window",
    )
    ep_carrier_id = fields.Many2one("easypost_base.carrier", string="Provider", compute="_get_easypost_carrier")
    preferred_service_ids = fields.Many2many(
        "easypost_base.service", "ep_carrier_service_rel", "carrier_id", "service_id", string="Preferred Services"
    )
    ...
    def _get_preferred_rate(self, ep_shipment):
        ...
        if not rates:
            message = _(
                "No rate satisfying your preferences were provided by the carrier. A "
                "frequent cause of this is an invalid address (street and number in "
                "particular)."
            )
            rate_error_msg = [m for m in ep_shipment.messages if m.get("type") == "rate_error"]
            if rate_error_msg:
                message += "\n"
                msg_dic = rate_error_msg[0]
                message += _('The message returned by {} was: "{}"').format(
                    msg_dic.get("carrier", _("the carrier")), msg_dic.get("message")
                )
...
class EasypostService(models.Model):
    ...
    @api.model
    def _setup_complete(self):
        super(EasypostService, self)._setup_complete()
