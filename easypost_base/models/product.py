...
from odoo.addons.easypost_base.models.carrier import EP_PREFIX, set_field_ep_carriers_selection
        ...
        with open(os.path.join(os.path.dirname(__file__), "../data/packages.py")) as packages_file:
            ...
            for carrier in packages_dic:
                ...
                for pack_name in packages_dic[carrier]:
                    exists = self.search([('package_carrier_type', '=', ep_carrier.code),
                                          ('name', '=', pack_name)])
                    if not exists and ep_carrier.installed:
                        self.sudo().create({'name': pack_name, 'shipper_package_code': pack_name,
